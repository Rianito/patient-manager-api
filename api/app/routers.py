...
def read_patient(request: Request, patient_cpf: CpfStr):
    result = request.app.database.patients.find_one({"cpf": patient_cpf}, {"_id": False})
    if result:
        return result
...
def update_patient(request: Request, patient: PatientUpdate, patient_cpf: CpfStr):
    ...
    if len(patient) >= 1:
        result = request.app.database.patients.update_one({"cpf": patient_cpf}, {"$set": patient})
...
def delete_patient(request: Request, patient_cpf: CpfStr):
    result = request.app.database.patients.delete_one({"cpf": patient_cpf})
